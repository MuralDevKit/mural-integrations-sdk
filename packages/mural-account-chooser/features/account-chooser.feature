Feature: account chooser
  ...
  Background:
    Given user AAD Token is available
    And the GET SESSION_STORAGE api response is 404 status

  Scenario: when requiring consent and no account for hint, both 'sign in with' and 'sign up from hint' are shown
    Given meta os user principal name is any_email@gmail.com
    ...
    """
    When I visit the "authentication wrapper" route
  ...
  Scenario: when not requiring consent and no account for hint, only 'sign up from hint' is shown
    Given meta os user principal name is any_email@mural.co
    And the POST REALM api response is 200 status with
    """
    {
      "accountStatus": 0,
  ...
  Scenario: when account exist for hint, 'sign in from hint' is shown
    Given meta os user principal name is existing@mural.co
    And the POST REALM api response is 200 status with
    """
    {
      "accountStatus": 2,
